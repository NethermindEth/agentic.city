"""Module implementing cryptocurrency and blockchain interactions for agents."""
...
class CryptoContext(Context):
    ...
    def __init__(self) -> None:
        """Initialize the crypto context with Web3 connection and key management."""
        self.tools.extend([self.get_balance, self.request_faucet])
        self.id = str(uuid4())
        ...
        # Setup faucet
        self.faucet_key_path = (
            Path(os.getenv("KEYS_DIRECTORY", "secure/keys")) / "app.key"
        )
    ...
    def get_context_instructions(self, agent: AgentIdentity) -> str:
        """Get instructions for using the crypto context.

        Args:
            agent: The agent requesting instructions.

        Returns:
            Instructions for using crypto tools.
        """
    ...
    def get_context(self, agent: AgentIdentity) -> Dict[str, Any]:
        """Get the current state of the crypto context.

        Args:
            agent: The agent requesting context.

        Returns:
            Current state of cryptocurrency operations.
        """
        if agent.id not in self.private_keys:
            self._load_key(agent.id)
        account = Account.from_key(self.private_keys[agent.id])
    ...
    @tool
    def get_balance(self, agent_identity: AgentIdentity, token: str = "eth") -> str:
    ...
    @tool
    def request_faucet(
        self, agent_identity: AgentIdentity, amount: float = 0.1  # Default to 0.1 ETH
    ) -> str:
