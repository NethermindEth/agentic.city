...
async def main() -> None:
    ...
    This function sets up logging, validates configuration, and handles the main bot
    lifecycle including graceful shutdown on interruption.
    """
    try:
...
def run() -> None:
    """Start the bot with proper asyncio event loop handling.

    Configures the event loop with appropriate settings for different platforms
    and manages the bot's execution lifecycle.
    """
    loop = None
    try:
